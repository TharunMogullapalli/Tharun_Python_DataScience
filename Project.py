...
Passengers = pd.read_csv("D:\\MBA\\Term-6\\Python & SPSS\\Air_Traffic_Passenger_Statistics.csv",sep=',')

Passengers.info()
Passengers.isna().sum()

Passengers_New = Passengers.drop(columns = ['Activity Period', 'Operating Airline IATA Code','Published Airline IATA Code', 'Published Airline'])
Passengers_New.info()

Y= Passengers_New.iloc[:,0]
print(Y.shape)

X= Passengers_New.iloc[:,1:8]
print(X.shape)
